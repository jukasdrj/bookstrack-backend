...
describe('CacheKeyFactory', () => {
  describe('authorSearch()', () => {
    test('should generate cache key with default parameters', () => {
      ...
      const expectedParams = btoa('maxResults=50&showAllEditions=false&sortBy=publicationYear').replace(/[/+=]/g, '_');
    ...
    test('should include custom maxResults parameter', () => {
      ...
      const expectedParams = btoa('maxResults=100&showAllEditions=false&sortBy=publicationYear').replace(/[/+=]/g, '_');
    ...
    test('should include custom sortBy parameter', () => {
      ...
      const expectedParams = btoa('maxResults=50&showAllEditions=false&sortBy=title').replace(/[/+=]/g, '_');
      expect(key).toContain(expectedParams);
    });
    ...
    test('should include showAllEditions parameter', () => {
      ...
      const expectedParams = btoa('maxResults=50&showAllEditions=true&sortBy=publicationYear').replace(/[/+=]/g, '_');
      expect(key).toContain(expectedParams);
    });
  ...
  describe('bookTitle()', () => {
    ...
    test('should handle special characters in title', () => {
      const key = CacheKeyFactory.bookTitle("Harry Potter & the Philosopher's Stone");
      expect(key).toBe("search:title:maxresults=20&title=harry potter & the philosopher's stone");
    });
  });
  ...
  describe('generic()', () => {
    test('should generate cache key with sorted parameters', () => {
      ...
      expect(key).toBe('custom:prefix:param1=value1&param2=value2&param3=value3');
    });
    ...
    test('should maintain consistent order regardless of input order', () => {
      const key1 = CacheKeyFactory.generic('prefix', { z: '1', a: '2', m: '3' });
      const key2 = CacheKeyFactory.generic('prefix', { m: '3', z: '1', a: '2' });
