...
describe("processAuthorBatch", () => {
  ...
  it("should skip already processed authors", async () => {
    env.CACHE.get.mockResolvedValueOnce(
      JSON.stringify({
        worksCount: 20,
        titlesWarmed: 18,
        lastWarmed: Date.now(),
  ...
  it("should track titlesWarmed count separately from worksCount", async () => {
    ...
    const processedCall = env.CACHE.put.mock.calls.find(
      (call) => call[0] === "warming:processed:author:neil gaiman",
    );
  ...
  it("should process titles in parallel with concurrency limit", async () => {
    ...
    // Verify concurrency limit was respected (should be 5 by default)
