...
/**
 * Internal request handler (extracted for analytics wrapping)
 * Handles all routing logic without analytics tracking
 */
async function handleRequest(request, env, ctx) {
  ...
  if (url.pathname === "/api/scan-bookshelf" && request.method === "POST") {
    ...
    try {
      ...
      console.log(`[API] Bookshelf scan scheduled via alarm for job ${jobId}`);
  ...
  if (url.pathname === "/search/advanced") {
    try {
      ...
      if (request.method === "GET") {
        ...
        authorName =
          url.searchParams.get("author") || url.searchParams.get("authorName");
...
export default {
  async fetch(request, env, ctx) {
    ...
    // Handle request and wrap with analytics tracking
